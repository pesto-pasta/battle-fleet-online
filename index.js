...
function getGameSubset(games, status, user) {
    ...
    let pendingArray;
    if (user) {
        pendingArray = keysArray.filter((key) => games[key].status === status && (user.username === games[key].invitee || user.username === games[key].inviter));
    }   else {
        pendingArray = keysArray.filter((key) => games[key].status === status);
    }
    
    const gameSubset = pendingArray.map(key => games[key]);
    if (status === GameStatus.PENDING && user) {
